...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    ...
    cos = torch.gather(cos.repeat(
        gather_indices.shape[0], 1, 1, 1), 2, gather_indices)
    sin = torch.gather(sin.repeat(
        gather_indices.shape[0], 1, 1, 1), 2, gather_indices)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        # Use the single projection layer and split its output into q, k, and v
        qkv_states = self.qkv_proj(hidden_states)
        query_states, key_states, value_states = torch.split(
            qkv_states, self.num_heads * self.head_dim, dim=2)
